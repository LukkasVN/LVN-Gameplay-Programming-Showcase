...
public class PlayerMovement : MonoBehaviour
{
    ...
    [SerializeField] private float ledgeWallOffset = 0.3f; // Distance from the wall (Have the CharacterController radius in mind)
    private bool _lastIdleLeft;
    private bool _isOnLedge;
    private bool _ledgeExitCooldown = false;
    private Transform _currentLedge;
    private Vector3 _ledgeForward; // direction along the ledge
    private Vector3 _ledgeInward;  // direction inward toward the wall
    private float _ledgeT; // Distance traveled along the ledge axis
    ...
    private void EnterLedge(Transform ledgeRoot)
    {
        ...
        _ledgeInward  = -_currentLedge.right; // adjust if outward is forward instead

        // Project current position onto ledge axis to set starting offset
        Vector3 localToLedge = transform.position - _currentLedge.position;
        _ledgeT = Vector3.Dot(localToLedge, _ledgeForward);

        // Rotation alignment (My mesh is rotated -180 on the Y axis by default so I adjust it here)
        ...
        // Animator fallback idle side determination
    ...
    private void ExitLedge()
    {
        ...
        Invoke(nameof(ResetLedgeCooldown), 0.2f); // Small cooldown to prevent immediate re-entry
    }
