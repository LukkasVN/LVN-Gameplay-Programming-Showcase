...
{
    if (DropState != EDropState::Floating 
        && DropState != EDropState::TransitioningToFloat
        && DropState != EDropState::Settled)  // <-- add this
            return;
...
{
    if (bPhysicsBased && bCollectDelayFinished)
    {
        UE_LOG(LogTemp, Log,
                TEXT("[DroppableItem] Physics-Based settled directly after delay"));
        MakeCollectible();
        return;
    }
    ...
    bool bReadyAfterDelay = bCollectDelayFinished && bVelocityLow;
...
void ADroppableItem::MakeCollectible()
{
    if (bPhysicsBased)
    {
        // Physics-based: keep physics running, no repositioning, no rotation snap.
        // Drop stays wherever it physically settled.
        DropState = EDropState::Settled;
        UE_LOG(LogTemp, Log, TEXT("[DroppableItem] %s settled (physics-based) and is now collectible!"),
            ItemData ? *ItemData->ItemName : TEXT("Unknown"));
        return;
    }

    // Floaty-Droppables path
    StopPhysicsClean();
