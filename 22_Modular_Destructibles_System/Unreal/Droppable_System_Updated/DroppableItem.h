...
public:
    ...
    UPROPERTY(EditDefaultsOnly, BlueprintReadOnly, Category = "Settle Settings", meta = (Tooltip = "If true, the drop stays wherever physics leaves it - no float transition, no bob, no rotation. Becomes collectible once Collect delay elapsed. FloatSettings is ignored when this is on."))
    bool bPhysicsBased = false;
