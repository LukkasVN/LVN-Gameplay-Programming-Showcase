...
#include "Destructible_System/IDestructible.h"
...
AFP_Character::AFP_Character()
{
    ...
    // Adds PlacementManager Component automatically.
    PlacementManager = CreateDefaultSubobject<UObjectPlacementManager>(TEXT("PlacementManager"));
}
...
void AFP_Character::Tick(float DeltaTime)
{
    ...
    // Destructible trace has a separate system, separate gate, separate range.
    // Range is driven by the active weapon: no weapon = nothing hittable = no hover.
    if (bCanHitFlag && ActiveWeapon)
    {
        UpdateDestructibleFocus();
    }
    else if (CurrentDestructibleFocus)
    {
        // Weapon unequipped or hit ability disabled mid-focus --> clear it cleanly.
        OnDestructibleUnfocused.Broadcast();
        CurrentDestructibleFocus = nullptr;
    }
...
void AFP_Character::SetupPlayerInputComponent(UInputComponent* PlayerInputComponent)
{
    ...
    if (ClickAction){
        EIC->BindAction(ClickAction, ETriggerEvent::Started, PlacementManager.Get(), &UObjectPlacementManager::OnConfirmInput);
    }

    if (CancelAction){
        EIC->BindAction(CancelAction, ETriggerEvent::Started, PlacementManager.Get(), &UObjectPlacementManager::OnCancelInput);
    }

    if (RotateAction)
    {
        EIC->BindAction(RotateAction, ETriggerEvent::Started, PlacementManager.Get(), &UObjectPlacementManager::OnRotateInput);
    }

    if (HitAction)
    {
        EIC->BindAction(HitAction, ETriggerEvent::Started, this, &AFP_Character::HandleHit);
    }
}
...
// Hit Logic Additions:
bool AFP_Character::CanHit() const
{
    return bCanHitFlag && bCanInteract && !bHitOnCooldown && ActiveWeapon != nullptr;
}

void AFP_Character::HandleHit()
{
    if (!CanHit()) return;
    ...
    GetWorldTimerManager().SetTimer(
        HitCooldownTimer,
        FTimerDelegate::CreateLambda([this]() { bHitOnCooldown = false; }),
        ActiveWeapon->HitCooldown,
        false);

    OnHitCooldownStarted.Broadcast(ActiveWeapon->HitCooldown);

    if (ActiveWeapon->bUseHitShake && ActiveWeapon->HitShakeClass)
    {
        if (APlayerController* PC = Cast<APlayerController>(GetController()))
        {
            if (PC->PlayerCameraManager)
                PC->PlayerCameraManager->StartCameraShake(ActiveWeapon->HitShakeClass);
        }
    }

    FHitResult Hit;
    AActor* Target = TraceForward(ActiveWeapon->HitRange, Hit);
    if (!Target) return;

    if (Target->GetClass()->ImplementsInterface(UDestructible::StaticClass()))
    {
        const FVector HitDir = (Hit.ImpactPoint - PlayerCamera->GetComponentLocation()).GetSafeNormal();
        IDestructible::Execute_TakeHit(Target, ActiveWeapon, Hit.ImpactPoint, HitDir);
    }
}

AActor* AFP_Character::TraceForward(float Distance, FHitResult& OutHit) const
{
    const FVector Start = PlayerCamera->GetComponentLocation();
    const FVector End = Start + PlayerCamera->GetForwardVector() * Distance;
...
void AFP_Character::UpdateDestructibleFocus()
{
    FHitResult Hit;
    AActor* HitActor = TraceForward(ActiveWeapon->HitRange, Hit);

    AActor* DestructibleHit = nullptr;
    if (HitActor && HitActor->GetClass()->ImplementsInterface(UDestructible::StaticClass()))
        DestructibleHit = HitActor;

    if (DestructibleHit == CurrentDestructibleFocus)
        return;

    if (CurrentDestructibleFocus)
        OnDestructibleUnfocused.Broadcast();
    ...
    if (DestructibleHit)
    {
        ...
        UDestructibleData* Data = IDestructible::Execute_GetDestructibleData(DestructibleHit);

        if (ADestructibleActor* destructible = Cast<ADestructibleActor>(DestructibleHit))
        {
            currentHP = destructible->GetCurrentHP();
        }

        OnDestructibleFocused.Broadcast(Data, currentHP);
    }

    CurrentDestructibleFocus = DestructibleHit;
}

void AFP_Character::SetCanHit(bool bEnabled) { bCanHitFlag = bEnabled; }
...
void AFP_Character::SetCanBob(bool bEnabled) { bCanBob = bEnabled; }
...
void AFP_Character::SetPlayerSystems(bool bEnabled)
{
    SetCanMove(bEnabled);
    SetCanLook(bEnabled);
    SetCanBob(bEnabled);
    SetCanInteract(bEnabled);
    SetCanJump(bEnabled);
    SetCanSprint(bEnabled);
    SetCanUseFlashlight(bEnabled);
    SetCanHit(bEnabled); 
